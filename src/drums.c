...
static void note_on(uint8_t note, uint8_t velocity) {
    ...
    if (midi_output_states[out].triggered) return;
...
void drum_task() {
    ...
    while (tuh_midi_stream_read(midi_dev_addr, &cable_num, pending_msg, sizeof(pending_msg)) != 0) {
    ...
    if (drum_state.flags & changed_flag && current_time - last_sent_time > ADAPTER_OUT_INTERVAL) {
        ...
        drum_state.input_pkt.handled = 0;
        drum_state.input_pkt.length = sizeof(xb_one_drum_input_pkt_t);
        OPENRB_DEBUG("WRITING TO QUEUE\r\n");
